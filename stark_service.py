...
def handle_all_messages(message):
    bot.reply_to(message, "🚀 جارٍ الفحص عبر Playwright...")
    ...
    try:
        ...
        # التأكد أن النتيجة ليست فارغة لتجنب إيرور 400
        if result and len(result.strip()) > 0:
            bot.reply_to(message, f"✅ النتيجة المسترجعة:\n{result[:900]}")
        else:
            bot.reply_to(message, "⚠️ الاستجابة فارغة تماماً من الخادم.")
