...
def handle_all_messages(message):
    bot.reply_to(message, "🔍 جاري الاتصال وجلب البيانات...")
    ...
    try:
        ...
        # التأكد إن النص مش فارغ وتجهيزه بشكل آمن تماماً لتليجرام
        if not result:
            result = "الاستجابة فارغة تماماً."
        
        # قص النص لو طويل عشان تليجرام يقبله بدون مشاكل
        safe_text = str(result)[:3500]
        bot.reply_to(message, f"📌 النتيجة:\n{safe_text}")
            
    except Exception as e:
        bot.reply_to(message, f"❌ حدث خطأ داخلي:\n{str(e)}")
